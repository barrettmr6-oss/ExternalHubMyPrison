...
-- Fire every ProximityPrompt found on/in an object
...
-- Teleport character to a position and wait briefly for server to register
...
-- Get the player's tycoon/prison folder in Workspace.
-- My Prison stores each player's tycoon under Workspace with a folder
-- named by their UserId or DisplayName, or under a "Tycoons" folder.
local function GetTycoon()
    -- Try common structures used by My Prison
    local uid = tostring(LocalPlayer.UserId)
    local name = LocalPlayer.Name

    -- Check Workspace.Tycoons first
    local tycoons = Workspace:FindFirstChild("Tycoons")
    if tycoons then
        local t = tycoons:FindFirstChild(uid) or tycoons:FindFirstChild(name)
        if t then return t end
        -- Some versions use a Value inside the folder to identify owner
        for _, folder in ipairs(tycoons:GetChildren()) do
            local owner = folder:FindFirstChild("Owner") or folder:FindFirstChild("OwnerID")
            if owner then
                local val = owner.Value
                if tostring(val) == uid or val == LocalPlayer then
                    return folder
                end
            end
        end
    ...
    -- Check direct Workspace children
    local direct = Workspace:FindFirstChild(uid) or Workspace:FindFirstChild(name)
    if direct then return direct end

    -- Last resort: find a folder with an Owner value matching the player
...
-- Scan a root (tycoon or whole workspace) for objects matching name keywords
local function FindObjects(root, keywords)
    local found = {}
...
-- =============================================
-- FEATURE LOGIC  (properly rewritten for My Prison tycoon)
-- =============================================

-- My Prison: trash/mess objects spawned inside your tycoon have prompts
-- The janitor role means the player IS the janitor - they walk to mess and
-- interact. The script teleports to each mess object and fires its prompt.
local function RunAutoClean()
    if not State.AutoClean then return end
    local root = GetTycoon() or Workspace
    -- My Prison mess objects: "Trash", "Dirty", "Mess", "Dish", "Laundry" etc.
    local keywords = {"trash","mess","dirt","litter","spill","dirty","dish","laundry","waste","debris","garbage"}
    local objects = FindObjects(root, keywords)
    for _, obj in ipairs(objects) do
        if not State.AutoClean then break end
        -- Only act on objects that have a ProximityPrompt (interactable)
        ...
        if hasPrompt then
            ...
            if pos then
                TeleportTo(pos)
                FireAllPrompts(obj)
                -- Also try remotes with the object as argument
...
-- My Prison: escape tunnels are dug by prisoners inside your prison area.
-- They appear as "Hole", "Tunnel" etc. with a prompt to fill them.
local function RunAutoFillTunnels()
    if not State.AutoFillTunnels then return end
    local root = GetTycoon() or Workspace
    local keywords = {"tunnel","hole","escapetunnel","escapehole","dighole","dig"}
    local objects = FindObjects(root, keywords)
    for _, obj in ipairs(objects) do
...
-- My Prison: criminals are NPC models in Crime City area of the map.
-- They are NOT other players. We find NPC models with criminal-type names
-- and fire their arrest ProximityPrompt, OR fire the Arrest remote with the NPC.
local function RunAutoArrest()
    if not State.AutoArrestCrim then return end
    local hrp = GetHRP()
    if not hrp then return end

    -- Crime City / street area - criminals spawn outside the prison
    -- Search entire workspace for criminal NPC models
    local criminalKeywords = {
        "criminal","gangster","robber","thief","suspect",
        "wanted","fugitive","convict","bandit"
    }

    -- Also look for any NPC with a "Wanted" attribute or tag
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if not State.AutoArrestCrim then break end
        if obj:IsA("Model") and obj ~= LocalPlayer.Character then
            ...
            -- Check for Wanted value/attribute
            if not isCriminal then
                local wantedVal = obj:FindFirstChild("Wanted") or obj:FindFirstChild("IsCriminal") or obj:FindFirstChild("Criminal")
                if wantedVal and (wantedVal.Value == true or wantedVal.Value == 1) then
                    isCriminal = true
                end
                if obj:GetAttribute("Wanted") or obj:GetAttribute("Criminal") then
                    isCriminal = true
                end
            end

            if isCriminal then
                local npcHRP = obj:FindFirstChild("HumanoidRootPart") or obj:FindFirstChild("RootPart")
                if npcHRP then
                    local dist = (hrp.Position - npcHRP.Position).Magnitude
                    if dist <= State.ArrestRadius then
                        FireAllPrompts(obj)
                        FireRemote("Arrest", obj)
                        FireRemote("ArrestCriminal", obj)
                        FireRemote("ArrestNPC", obj)
                        task.wait(0.15)
                    else
                        -- Teleport close to criminal and arrest
...
-- My Prison: buffets/food stations inside tycoon need restocking.
-- Chef role: teleport to food objects and interact.
local function RunAutoFeed()
    if not State.AutoFeedPrisoners then return end
    local root = GetTycoon() or Workspace
    local keywords = {"buffet","food","tray","serving","counter","chef","kitchen","fridge","oven"}
    local objects = FindObjects(root, keywords)
    for _, obj in ipairs(objects) do
...
-- My Prison: Fire Event (2026 update) - fires can break out inside the prison.
-- Staff-only door involved. Find fire objects and extinguish via prompt.
local function RunAutoExtinguish()
    ...
    for _, obj in ipairs(root:GetDescendants()) do
        ...
        -- Actual Fire particle
        if obj:IsA("Fire") then
            isFire = true
        end

        -- BasePart/Model with fire-related name
        if not isFire and (obj:IsA("BasePart") or obj:IsA("Model")) then
            local lower = obj.Name:lower()
            local fireKW = {"fire","burn","flame","blaze","ignite","firezone","firehazard"}
        ...
        if isFire then
            ...
            elseif base:IsA("Model") then
                local ok, cf = pcall(function() return base:GetModelCFrame() end)
                if ok then pos = cf.Position end
            end
...
-- My Prison: Contraband (2026) - shivs, drugs etc found on prisoners or ground.
-- Confiscate by proximity prompt interaction.
local function RunAutoContraband()
    if not State.AutoContraband then return end
    local root = GetTycoon() or Workspace
    local keywords = {"contraband","shiv","drug","weapon","knife","illegal","illegalitem","smuggle"}
    local objects = FindObjects(root, keywords)
    for _, obj in ipairs(objects) do
        ...
        if pos then
            ...
            FireRemote("RemoveContraband", obj)
            task.wait(0.2)
        end
    end
end

-- Master scan loop - runs each feature on its own thread
-- Uses ScanRateRaw (integer) / 10 = seconds, so value 12 = 1.2s
local lastTick = 0
RunService.Heartbeat:Connect(function()
    ...
    if State.AutoArrestCrim    then task.spawn(RunAutoArrest)      end
    if State.AutoFeedPrisoners then task.spawn(RunAutoFeed)        end
    if State.AutoExtinguish    then task.spawn(RunAutoExtinguish)  end
    if State.AutoContraband    then task.spawn(RunAutoContraband)  end
end)
...
MakeInfoCard(ip, {
    "Guards: Arrest criminal NPCs, Extinguish fires, Confiscate",
    "Staff: Clean trash, Fill tunnels, Restock food buffets",
    "Main: Quick access to 3 most-used toggles",
    "Teleports to each target, fires prompt AND server remote",
    "Tycoon folder auto-detected from your UserId",
    "Radius: 10-120 studs controls how far to scan for criminals",
})
